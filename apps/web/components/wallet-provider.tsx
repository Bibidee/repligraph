...
const disconnect=async()=>{const e=(window as any).ethereum;setError(null);if(e){try{await e.request({method:'wallet_revokePermissions',params:[{eth_accounts:{}}]})}catch(_err){/* Wallets without revocation support fall back to local state. */}}setAccount(null);setChainId(null)};
...
useEffect(()=>{const e=(window as any).ethereum;if(!e)return;const ac=(a:string[])=>setAccount(a?.[0]??null);const ch=(c:string)=>setChainId(c);const dc=()=>{setAccount(null);setChainId(null)};e.on?.('accountsChanged',ac);e.on?.('chainChanged',ch);e.on?.('disconnect',dc);return()=>{e.removeListener?.('accountsChanged',ac);e.removeListener?.('chainChanged',ch);e.removeListener?.('disconnect',dc)}},[]);

...
export async function getStudy(id:string):Promise<Study|null>{return readContract('get_study',[Number(id)])}
export async function getStudyVersion(id:string,version:number):Promise<Study|null>{return readContract('get_study_version',[Number(id),Number(version)])}
export async function listStudies(offset=0,limit=50):Promise<Study[]>{return readContract('list_studies',[offset,limit])}
export async function getRelation(id:string):Promise<RelationClaim|null>{return readContract('get_relation',[Number(id)])}
